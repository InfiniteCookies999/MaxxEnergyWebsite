...
function setCounties(dropdown, key, userCounty) {
  ...
  let selectedCounty = userCounty;
  for (const county of countyList) {
    const countyValue = county.replaceAll(" ", "-");
    if (selectedCounty === "") {
      selectedCounty = countyValue;
    }
    // TODO: should probably change the key that is used here.
    dropdown.append(`<option value=${countyValue}>${county}</option>`);
  }
...
async function addStatesAndCounties(stateDropdown, countyDropdown, userState, userCounty) {
  ...
  for (const state of STATES) {
    ...
    stateDropdown.append(`<option value=${state.abbreviation}>${name}</option>`);
  }
